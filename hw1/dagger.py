...
if __name__ == '__main__':
            ...
            for iter in range(ITERATION):
                ...
                for epoch in range(1):
                    ...
                    while not done:
                        ...
                        action = policy_fn(s[np.newaxis, :])

                        train = np.vstack((train, s[np.newaxis, :]))
                        label = np.vstack((label, action))

                roll_reward = []
                for epoch in range(NUM_ROLLOUTS):
                    s = env.reset()
                    done = False
                    reward = 0
                    step = 0
    ...
    with open(os.path.join("test_data", "dagger_" + ENV_NAME+".pkl"), "wb") as f:
