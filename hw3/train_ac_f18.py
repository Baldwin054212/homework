...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_HW2 CODE_HERE
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, "nn_actor", self.n_layers, self.size, tf.tanh)
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # YOUR_HW2 CODE_HERE
            sy_log_a = tf.nn.softmax(sy_logits_na, 1)

            sy_sampled_ac = tf.squeeze(tf.multinomial(sy_log_a, 1), -1)
        else:
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            sy_logprob_n = tf.reduce_sum(sy_logits_na*ac_onehot, axis=1)
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_HW2 CODE_HERE
            sy_logprob_n = None
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(self.num_target_updates):
            ...
            for j in range(self.num_grad_steps_per_target_update):
                closs, _ = self.sess.run([self.critic_loss, self.critic_update_op], feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: target})
        return closs
...
def train_AC(
    ...
    for itr in range(n_iter):
        ...
        aloss = agent.update_actor(ob_no, ac_na, adv)
